...
await card.locator('.pc-src').filter({ hasText: names[0] }).waitFor({ timeout: 15000 })
const coverOf = designs.find((x) => x.doc_id === ids[0]).cover_path
await card.locator(`#pcCoverPrev img[src="/quadro/covers/${coverOf}"]`).waitFor({ timeout: 15000 })
check('确认卡片的封面就是这一座的画面', true)
const title = `导入封面 ${Date.now().toString(36)}`
await card.locator('input[name=title]').fill(title)
const age = card.locator('select[name=age_min_m]')
check('起步月龄按量出来的高度给了建议值', !!(await age.inputValue()))
await card.locator('input[name=kid_m]').fill('30')
await card.locator('textarea[name=why]').fill('官方软件里画好的作品，批量导入以后发出来。')
await card.locator('input[name=age_ok]').check()
...
if (me.status !== 'approved') {
  ...
  await ap.locator('.card.rv').filter({ hasText: '导入联调设计师' }).getByRole('button', { name: '通过', exact: true }).click()
  await ap.locator('.flash.ok').waitFor()
  await admin.close()
  me = await json(await ctx.request.get('/quadro/pros/me'), 'pros/me')
}
check('pro-designer 是认证设计师', me.status === 'approved')
const home = await json(await ctx.request.get(`/quadro/pros/${me.id}`), 'pros/{id}')
const work = home.works.find((w) => w.slug === scheme.slug)
check('设计师主页的作品里有这一座，封面是同一张', !!work && work.cover_url === scheme.cover_url)
const list = await json(await ctx.request.get('/quadro/pros'), 'pros')
const item = list.pros.find((p) => p.id === me.id)
check('设计师页这位设计师的作品缩略图里有这一座', !!item && item.covers.includes(scheme.cover_url))

const pro = await ctx.newPage()
await pro.goto(`/pro.html?id=${me.id}`)
...
await pro.goto(`/pros.html?q=${encodeURIComponent('导入联调设计师')}`)
await pro.locator(`img[src="${scheme.cover_url}"]`).first().waitFor({ timeout: 15000 })
