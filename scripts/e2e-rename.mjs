...
async function run() {
  ...
  await pub.locator('#coverPrev img[src^="data:"]').waitFor({ timeout: 20_000 })
  await pub.locator('#dimsAuto').waitFor({ state: 'visible', timeout: 20_000 })
  await pub.locator('input[name=title]').fill(`${RUN} 双层游戏屋`)
  await pub.locator('select[name=cat]').selectOption('climb')
  await pub.locator('input[name=kid_m]').fill('30')
  await pub.locator('[data-train="climb"]').click()
  await pub.locator('textarea[name=why]').fill('下层是小客厅，上层平台加斜顶，两个孩子一上一下不抢地方。')
