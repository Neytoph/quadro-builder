...
3. **对库存**
  「零件清单」看用了什么；「我的库存」填手里的数量。「选购建议」会列出还缺哪些件、该买哪套。零件清单底下能把料表导成表格（CSV，Excel 直接开，填过库存会多「拥有」「还缺」两列）或一张图片（带缩略图，发群里直接看）。
4. **看安全**
...
5. **轮 / 布 / 泳池 / 45° / 双管 / 管夹 / 孔锁 / 柔性接头**：从顶栏拿出，吸到高亮点上。双管夹点一下转 45°，拖着绕管可转到任意角（Shift 按 15° 一档）；选择模式里选中夹子同样能转，同一根管子上带着第二根管的夹子会一起转。第二根管可沿第一根平移（夹子不动），端点会吸到第一根上。泳池下拉的最后一项是**海洋球**：点一个泳池倒进去，再点一次倒出来；料表按一袋 500 个算袋数，.qdf 里没有这一项。
   **兼容件**（国产件，不是 QUADRO 原厂）：面板下拉里的功能板（乐高、篮球、蜂窝、忙碌、毛毡、磁吸、攀岩、感官垫、布兜、感官盆），布件下拉里的彩虹带、彩虹桥和软包滚筒。料表里都标着「兼容件」，价格空着。导出 .qdf 时功能板、彩虹带、彩虹桥写成普通板 / 布件并在材质名里做标记，官方软件照常打开，结构一样；软包滚筒和海洋球不写进去。
...
| 旋转选中    | Q 逆时针 / E 顺时针                     |
| 转弯管     | 只选中一根弯管时，Q / E 绕它接着的那头转 90°；另一头还接着东西就转不了，先拆开 |
| 镜像选中    | M 左右 / ⇧M 前后，按当前视角定方向；粘贴中翻手上的副本。弯滑梯没有镜像款，含它的选中会被拒绝 |
| 成组 / 解组 | ⌘G / ⌘⇧G。组里任何一件被点到整组一起选；存进设计和分享链接，.qdf 里没有 |
